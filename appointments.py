...
class AppointmentsPage:
    ...
    def create_widgets(self):
        ...
        tk.Label(header_frame, text="Appointment Management", font=('Arial', 24, 'bold'), bg='#2c3e50', fg='white').pack(side='left', padx=20, pady=20)
        ...
        tk.Button(control_frame, text="Search", font=('Arial', 10), bg='#3498db', fg='white', relief='flat', cursor='hand2', command=self.search_appointments).pack(side='left', padx=(10,0))
        tk.Button(control_frame, text="Schedule Appointment", font=('Arial', 12, 'bold'), bg='#27ae60', fg='white', relief='flat', cursor='hand2', command=self.add_appointment).pack(side='left', padx=5)
        tk.Button(control_frame, text="Edit Appointment", font=('Arial', 12), bg='#f39c12', fg='white', relief='flat', cursor='hand2', command=self.edit_appointment).pack(side='left', padx=5)
        tk.Button(control_frame, text="Cancel Appointment", font=('Arial', 12), bg='#e74c3c', fg='white', relief='flat', cursor='hand2', command=self.cancel_appointment).pack(side='left', padx=5)
    ...
    def load_appointments(self):
        try:
            ...
            # Map IDs to names
            patients = {p['id']: p['full_name'] for p in self.app.db.get_all_patients()}
            doctors = {d['id']: d['full_name'] for d in self.app.db.get_all_doctors()}

            for appt in appointments:
                self.appointments_tree.insert('', 'end', values=(
                    appt.get('id', ''),
                    f"{appt.get('patient_id')} - {patients.get(appt.get('patient_id'), '')}",
                    f"{appt.get('doctor_id')} - {doctors.get(appt.get('doctor_id'), '')}",
    ...
    def appointment_form_window(self, title, appointment_data=None):
        ...
        # --- Form Fields ---
        fields = {}
        tk.Label(scroll_frame, text=title, font=('Arial',18,'bold'), bg='white', fg='#2c3e50').pack(pady=10)
        for label, key, widget in [
            ...
            tk.Label(scroll_frame, text=label, font=('Arial',12,'bold'), bg='white', fg='#2c3e50').pack(anchor='w', pady=(10,5))
            if widget == tk.Text:
        ...
        try:
            ...
            self.patient_map = {f"{p['id']} - {p['full_name']}": p['id'] for p in patients}
            fields['patient']['values'] = list(self.patient_map.keys())

            doctors = self.app.db.get_all_doctors()
            self.doctor_map = {f"{d['id']} - {d['full_name']}": d['id'] for d in doctors}
        ...
        if appointment_data:
            ...
            # Extract patient_id and doctor_id from Treeview values
        ...
        tk.Button(btn_frame, text="Save", font=('Arial',12,'bold'), bg='#27ae60', fg='white',
                  relief='flat', cursor='hand2',
                  command=lambda: self.save_appointment(form_window, fields, appointment_data)).pack(side='left', padx=10, ipadx=20, ipady=8)
        tk.Button(btn_frame, text="Cancel", font=('Arial',12), bg='#95a5a6', fg='white',
                  relief='flat', cursor='hand2', command=form_window.destroy).pack(side='left', ipadx=20, ipady=8)
    ...
    def save_appointment(self, window, fields, appointment_data=None):
        try:
            ...
            # Get actual IDs from map
            data['patient_id'] = self.patient_map.get(patient_selection)
            data['doctor_id'] = self.doctor_map.get(doctor_selection)
            ...
            data['notes'] = fields['notes'].get('1.0', 'end').strip()

            required_fields = ['patient_id','doctor_id','appointment_date','appointment_time','status']
            for f in required_fields:
                if not data[f]:
                    messagebox.showerror("Error", f"Please fill {f}")
                    return
